...
체크포인트 탐색(있는지 찾기)과 후보 수집(공감 판단 대상 모으기)을 페이지 하나당 한 번의
조회로 같이 처리 — 페이지를 2번 요청하는 낭비 방지 목적. 체크포인트를 찾을지는 끝까지
가봐야 알 수 있으므로, 일단 전부 수집해두고 결과에 따라 사후 절단 처리.

1. 최신 글부터 페이지 단위로 조회하며, 페이지마다 체크포인트 글과 ID가 일치하는지 검사와
   동시에 일치하지 않는 글을 전부 후보로 수집. 최대 체크포인트 탐색 상한(200페이지)까지
   계속 — 게시판이 비거나(빈 페이지) 상한 소진까지 대상.
   - 200페이지까지 넓게 보는 근거: 봇 재개 시점에 따라 체크포인트 글이 5페이지보다 뒤에
     위치 가능. 5페이지로 제한하면 아직 살아있는 글을 "삭제 추정"으로 오판, 5페이지
     이후 쌓인 새 글 전체 누락 위험.
2. 체크포인트를 만나면 `checkpoint_found = True`, 만난 시점까지 모은 후보 그대로 사용
   (사후 절단 없음 — 페이지 수 제한 없이 체크포인트 직전까지 전부 후보).
3. 200페이지를 다 돌아도 못 만나면(글 삭제 추정) `checkpoint_found = False` 처리 후,
   모아둔 후보를 `bot.max_pages`(5)페이지 분량으로 사후 절단 — 탐색은 200페이지까지
   해도 처리 대상까지 200페이지로 두면 체크포인트를 잃을 때마다 대량 처리가 발생해
   애초에 `bot.max_pages`를 둔 안전장치 취지와 어긋난다. `scan_limit_reached`는 5페이지
   상한 도달 시 `True`.
